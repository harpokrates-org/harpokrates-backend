...
const {
  ...
  UnknownUserError,
  PhotoNotFoundError,
} = require("../errors/FlickerWrapperErrors");
...
class FlickrWrapper {
  ...
  async getUserProfile(userID) {
    try {
      ...
      return {
        ...
        realname: body.person.realname
          ? body.person.realname._content
          : body.person.username._content,
  ...
  async getPhotos(userID, perPage = 100) {
    try {
      ...
      const photos = body.photos.photo.map((p) => ({
        id: p.id,
        title: p.title,
      }));
      return photos;
  ...
  async getUsersWhoHaveFavorited(
    mainUsername,
    photoIDs,
    photosPerFavorite,
    depth = GRAPH_DEAPTH,
    mutex = new Mutex(),
    nodes = new Set([mainUsername]),
    edges = new Set(),
    queue = [mainUsername]
  ) {
    try {
      ...
      const otherProm = body.photo.person.map(async (user) => {
        ...
        if (!nodes.has(user.username)) {
          ...
          try {
            ...
            return await this.getUsersWhoHaveFavorited(
              user.username,
              userPhotoIDs,
              photosPerFavorite,
              depth,
